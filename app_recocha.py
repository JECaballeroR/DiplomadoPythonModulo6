...
radio_button=st.sidebar.radio(label='Opciones', options=['Decir Holi', 'No decir Holi'])
...
opcion_sales=st.sidebar.selectbox(label="Seleccione un valor de 'sales'",
                                  options=lista_sales_unicos)
st.markdown("---")
...
opcion_y=st.sidebar.selectbox(label="Seleccione una variable",
                                  options=otras_variables)
@st.cache
def plot_simple(datos, x, y, sales_filter):
    data=datos.copy()
